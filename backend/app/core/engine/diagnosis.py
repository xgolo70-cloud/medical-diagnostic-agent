...
class DiagnosisEngine:
    ...
    def generate_diagnosis(self, patient_data: PatientData, lab_results: str = "") -> dict:
        ...
        try:
            ...
            # Post-process to match frontend schema
            if "differential_diagnosis" in result:
                for i, dx in enumerate(result["differential_diagnosis"]):
                    # Generate ID
                    if "diagnosis_id" not in dx:
                        dx["diagnosis_id"] = f"DX-{datetime.datetime.now().strftime('%H%M')}-{i+1}"
                    
                    # Map condition -> primary_diagnosis
                    if "condition" in dx and "primary_diagnosis" not in dx:
                        dx["primary_diagnosis"] = dx.pop("condition")
            ...
            if "citations" not in result:
                result["citations"] = []
            if "clinical_notes" not in result:
                result["clinical_notes"] = "Generated by MedGemma AI."
