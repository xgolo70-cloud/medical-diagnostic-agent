...
from app.api import ingest, diagnose, history, auth, dashboard, admin, health, patients
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting Medical Diagnostic Agent API...")
    init_database()
    yield
    logger.info("Shutting down Medical Diagnostic Agent API...")
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # CSP is intentionally omitted for API responses.
        # It should be set by the frontend server for HTML pages.
        return response
...
# CORS must be the LAST middleware added so it executes FIRST (outermost).
# This ensures OPTIONS preflight requests are handled before rate limiting.
app.add_middleware(
    CORSMiddleware,
